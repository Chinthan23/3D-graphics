	...
	render(scene, shader) 
	{
		// if(shader.type==="normal"){
			...
			if(shader.type==="normal"){
				...
				shader.setUniform3f("uLight",scene.field.light);
				...
				scene.models.forEach( function (model) {
					if(model.catcher || model.move){

						model.arrows.forEach( function (arrow) {
							//Draw arrow for each model as main axis.
							arrow.transform.updateModelTransformMatrix();
							shader.bindArrayBuffer(shader.vertexAttributesBuffer, arrow.vertices);
							shader.fillAttributeData("aPosition", 3,  
							// 3 * model.arrow.vertices.BYTES_PER_ELEMENT, 
							0,
							0);		
							
							shader.setUniform4f("uColor", arrow.color);
							shader.setUniform3f("uLight",model.light);
							
							shader.setUniformMatrix4fv("uModelTransformMatrix", arrow.transform.modelTransformMatrix);
							shader.setUniformMatrix4fv("uViewMatrix",scene.viewMatrix)
							shader.setUniformMatrix4fv("uProjectionMatrix",scene.projectionMatrix)
							shader.bindIndexBuffer(shader.indexBuffer,arrow.indices);
							shader.fillAttributeData("aNormal",3,3*arrow.vertexNormals.BYTES_PER_ELEMENT,0);
							// Draw
							shader.drawElements(arrow.indices.length);
						})
					}
				});
			}
			...
			scene.models.forEach( function (model) {
				...
				shader.type==="normal"?shader.setUniform3f("uLight", model.light):"";
